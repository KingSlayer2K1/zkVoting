...
Patna, India}
\and
\IEEEauthorblockN{Durjoy Majumdar}
\IEEEauthorblockA{\textit{Department of Computer Science and Engineering}\\
\textit{Indian Institute of Technology Patna}\\
Patna, India}
}
...
O($n$) tally-time nullification.  Second, we conduct a systematic audit
identifying 13 security assumptions in the original protocol, and
implement six novel improvements addressing five of them:
(1) a concrete $\pi_{\text{total}}$ proof using the
...
Feldman VSS verification and threshold partial-decryption tally; (3)
election-scoped key derivation for cross-election unlinkability; (4)
ephemeral epoch secrets for forward secrecy against retroactive
deanonymisation; (5) commit-reveal registration for censorship resistance;
and (6) a hidden re-vote grace period to prevent timing-based coercion
attacks.  We provide a detailed comparative security and performance analysis
between the original protocol and our enhanced system.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Systematic assumption audit}: identification and categorisation
    of 13 security assumptions in the original protocol, ranked by severity
    and real-world impact (Section~\ref{sec:assumptions}).
  ...
  \item \textbf{Cross-election unlinkability} (Improvement~III): election-scoped
    ...
    linking a voter's participation across elections.
  \item \textbf{Forward secrecy} (Improvement~IV): per-election ephemeral
    secrets in the serial number derivation prevent retroactive
    deanonymisation even if the voter's master secret is later compromised.
  \item \textbf{Registration censorship resistance} (Improvement~V):
    commit-reveal registration with a public audit trail makes authority
    censorship cryptographically detectable.
  \item \textbf{Re-vote grace period} (Improvement~VI): a hidden grace
    window after the public deadline allows coerced voters to re-vote,
    closing timing-based coercion attacks.
...
$\Pi_{\mathrm{NC}}$ comprises eight algorithms:
\textsf{Setup}, \textsf{KeyGen}, \textsf{Commit}, \textsf{Nullify},
\textsf{Open}, $\textsf{Open}_{\mathrm{null}}$, \textsf{KeyProve},
and \textsf{SimKeyProve}.  It operates over a prime-order elliptic curve group
...
\[
  R_{\text{total}} = \bigl\{(\mathit{ck}^*, \mathit{cm}, n_v\,;\, \mathit{msk})\bigr\}
\]
where $\mathit{cm}^* \leftarrow \mathsf{Nullify}(\mathit{msk}, \mathit{cm})$
and $\mathsf{Open}_{\mathrm{null}}(\mathit{ck}^*, \mathit{cm}^*, n_v, 0) = 1$.
...
(Table~3) but provides no concrete implementation.
...
After nullification:
\begin{align*}
  \mathit{cm}^*_{\mathrm{agg}} &= C_{2,\text{agg}} - \mathit{msk}\cdot C_{1,\text{agg}}\\
  &= \textstyle\sum_i h_{2,i} - \mathit{msk}\cdot \sum_i h_{1,i}\\
  &= \textstyle\sum_i (h_{2,i} - \mathit{msk}\cdot h_{1,i})\\
  &= \textstyle\sum_i b_i \cdot g_3 = n_v^{\text{real}}\cdot g_3
\end{align*}
...
argument (Fiat-Shamir).  Sample $t \leftarrow\!\!\!\$\ \mathbb{Z}_q$, then:
{\small
\begin{align*}
  A_1 &= t\cdot g_1,\quad A_2 = {-t}\cdot C_{1,\text{agg}}\\
  c &= H(g_1,\, g_4,\, C_{1,\text{agg}},\, \mathit{cm}^*_{\text{agg}},\\
    &\qquad A_1,\, A_2,\, n_v)\\
  k &= t + \mathit{msk}\cdot c \bmod q
\end{align*}
}
\noindent Verifier checks:
$k\cdot g_1 = A_1 + c\cdot g_4$
and
$-k\cdot C_{1,\text{agg}} + C_{2,\text{agg}} - \mathit{cm}^*_{\text{agg}}$
$= A_2 + c\cdot(C_{2,\text{agg}} - \mathit{cm}^*_{\text{agg}})$.
...
\vspace{4pt}
\noindent\textbf{Split.}
Choose random polynomial over $\mathbb{Z}_q$:
\[
  f(x) = \mathit{msk} + a_1 x + \cdots + a_{t-1}x^{t-1}
\]
Share $i = (i,\, f(i))$ for $i = 1,\ldots,n$.
...
Public commitments $\{C_j = a_j\cdot\mathsf{Base8}\}_{j=0}^{t-1}$
allow each authority to verify its own share without trusting the dealer:
\[
  s_i\cdot\mathsf{Base8} = \textstyle\sum_{j=0}^{t-1} C_j\cdot i^j
\]
...
\subsection{Improvement IV: Forward Secrecy via Ephemeral Epoch Secrets}

\textbf{Limitation in the paper.}
The voter's identity secret $\mathit{sk}_{\mathrm{id}}$ is a permanent
credential.  If it is ever compromised---through device theft, malware, or
legal compulsion---the attacker can compute the serial number
$\mathit{sn} = H(e, \mathit{ck}, \mathit{sk}_{\mathrm{id}})$ for \emph{any}
past or future election.  This enables retroactive deanonymisation: the
attacker links all of the voter's historical ballots to their identity.
The paper provides no mechanism for forward secrecy.

\textbf{Our implementation.}
We add a per-election \emph{ephemeral secret} $\sigma_e$ to the serial number
derivation:
\[
  \mathit{sn} = H(e,\; \mathit{ckHash}_e,\; \mathit{sk}_{\mathrm{id}},\; \sigma_e)
\]
The voter generates $\sigma_e$ fresh for each ballot and \emph{deletes it
immediately after voting}.  The circuit change is minimal: the serial number
hasher moves from $\mathrm{Poseidon}(3)$ to $\mathrm{Poseidon}(4)$, with
$\sigma_e$ as an additional private input.

\vspace{4pt}
\noindent\textbf{Security argument.}
\begin{itemize}
  \item If $\mathit{sk}_{\mathrm{id}}$ leaks \emph{after} $\sigma_e$ is
    deleted, the attacker cannot recompute past serial numbers (Poseidon
    preimage resistance).
  \item The public key $\mathit{pkid} = H(\mathit{sk}_{\mathrm{id}})$ is
    unchanged, so voter identity binding and Merkle eligibility are unaffected.
  \item The cost is one additional Poseidon hash input per proof --- negligible
    compared to the Merkle path verification.
\end{itemize}

Exposed in the circuit via \texttt{signal input epochSecret} and computed
in the backend at vote-casting time.

\subsection{Improvement V: Registration Censorship Resistance}

\textbf{Limitation in the paper.}
The authority controls which voters enter the Merkle tree $\mathit{rt}$.
A malicious authority can silently exclude a voter's $(ckHash, pkid)$ pair.
The voter's ZK proof will fail (Merkle membership check), and there is no
mechanism to prove the exclusion was censorship rather than a legitimate
rejection.

\textbf{Our implementation.}
We introduce a \emph{commit-reveal registration} protocol:
\begin{enumerate}
  \item \textbf{Commit.} Before registration, the voter publishes a commitment:
    \[
      \mathit{regCommit} = H(\mathit{ckHash},\; \mathit{pkid},\; \mathit{nonce})
    \]
    to a public append-only log.
  \item \textbf{Reveal.} The voter reveals $(\mathit{ckHash}, \mathit{pkid},
    \mathit{nonce})$ during normal registration.  The authority verifies
    the hash matches $\mathit{regCommit}$
    and inserts into the Merkle tree.
  \item \textbf{Audit.} Any observer can compare the commit log against
    the Merkle tree entries.  A valid commit with no corresponding Merkle
    entry constitutes \emph{cryptographic evidence of censorship}.
\end{enumerate}

This makes registration censorship \emph{publicly detectable} with zero
false positives.  The commit log is append-only (JSON file in prototype;
a blockchain transaction in production).

Exposed at \texttt{POST /api/register/commit} and \texttt{GET /api/register/audit}.

\subsection{Improvement VI: Re-Vote Grace Period}

\textbf{Limitation in the paper.}
Coercion resistance requires the voter to re-vote with their real key after
being coerced.  A sophisticated coercer can time the attack to $T - 1\min$
before the public voting deadline, leaving the voter no window to re-vote.
The paper assumes synchronous phases but does not address timing-based
coercion attacks near phase boundaries.

\textbf{Our implementation.}
We introduce a \emph{hidden grace period} after the public voting deadline:
\begin{itemize}
  \item The election has two deadlines: $T_{\mathrm{public}}$ (announced) and
    $T_{\mathrm{grace}}$ (secret, known only to the system).
  \item $T_{\mathrm{grace}} > T_{\mathrm{public}}$ (default: 60 minutes).
  \item During the grace window $[T_{\mathrm{public}}, T_{\mathrm{grace}})$,
    re-votes with the \emph{same serial number} overwrite the previous ballot
    (last-write-wins).  New first-time votes are rejected.
  \item The coercer believes the election ended at $T_{\mathrm{public}}$;
    the voter can secretly re-vote during the grace window.
  \item $T_{\mathrm{grace}}$ is never exposed via the public API.
\end{itemize}

This closes the timing attack vector at the cost of a brief (configurable)
delay before tally publication.

Exposed at \texttt{POST /api/election/init} with the \texttt{graceMinutes}
parameter.

% ─────────────────────────────────────────────────────────────────────────────
\section{Systematic Assumption Analysis}
\label{sec:assumptions}
% ─────────────────────────────────────────────────────────────────────────────

We conducted a systematic audit of the security assumptions made by the
zkVoting paper.  We identified 13 assumptions, categorised by severity and
real-world impact.  Table~\ref{tab:assumptions} summarises all 13; those
addressed by our improvements are marked.

\begin{table*}[t]
\centering
\caption{Systematic assumption analysis of the zkVoting protocol}
\label{tab:assumptions}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{clllc}
\toprule
\textbf{ID} & \textbf{Assumption} & \textbf{Severity} & \textbf{If Violated} & \textbf{Addressed} \\
\midrule
A1  & No on-chain Sybil prevention         & Critical & One person, many votes          & --- \\
A2  & Private untappable channel           & Critical & Coercion resistance fails       & --- \\
A3  & Re-vote timing near deadline        & High     & Timing-based coercion wins      & Impr.~VI \\
A4  & Authority registration fairness      & Critical & Ballot stuffing / censorship     & Impr.~I+V \\
A5  & Groth16 trusted setup               & High     & Fake proofs possible            & --- \\
A6  & Threshold collusion ($<t$ honest)    & Critical & Full msk reconstructed          & Impr.~II \\
A7  & DDH/DLP quantum hardness            & Medium   & All EC crypto broken            & --- \\
A8  & No forward secrecy on $\mathit{sk}$  & High     & Retroactive deanonymisation     & Impr.~IV \\
A9  & Poseidon random oracle model         & Medium   & Commitment binding may fail     & --- \\
A10 & Blockchain as bulletin board         & High     & Tampering with tallied votes     & --- \\
A11 & Cross-election key linkability       & Medium   & Voter tracking across elections & Impr.~III \\
A12 & Synchronous phase boundaries         & Low      & Late votes rejected             & --- \\
A13 & DLP-based tally $O(n)$ scaling       & Low      & Large-scale elections impractical & --- \\
\bottomrule
\end{tabular}
\end{table*}

Of the 13 assumptions, our six improvements directly address five (A3, A4, A6, A8, A11).
Section~\ref{sec:future} discusses proposed solutions and rationale for the remaining
eight unaddressed assumptions.
...
Forward secrecy (Impr.~IV)   & $\times$   & $\times$   & \checkmark \\
Reg.\ censorship detect.\ (V) & $\times$   & $\times$   & \checkmark \\
Re-vote grace period (VI)    & $\times$   & $\times$   & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
Cross-election unlinkability & $\times$ & $\times$ & \checkmark \\
Forward secrecy ($\sigma_e$) & $\times$ & $\times$ & \checkmark \\
Commit-reveal registration  & $\times$ & $\times$ & \checkmark \\
Hidden grace period          & $\times$ & $\times$ & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \texttt{registrationAudit.js}: commit-reveal registration with censorship audit (Improvement~V)
  \item \texttt{enhanced\_vote.circom}: full $R_{\text{vote}}$ circuit with
    pkid proof, Merkle membership, and epochSecret (Improvement~IV)
\end{itemize}

\subsection{New API Endpoints}

\begin{itemize}
  \item \texttt{POST /api/admin/reset} --- wipe data for fresh experiment (Dashboard)
  \item \texttt{GET /api/admin/voters} --- full voter registry (Dashboard)
  ...
  \item \texttt{POST /api/register/commit} --- registration commitment (Improvement~V)
  \item \texttt{GET /api/register/audit} --- censorship detection audit (Improvement~V)
\end{itemize}

\subsection{Research Simulation Dashboard}

To facilitate protocol evaluation and simulate diverse election scenarios, we developed a comprehensive web-based dashboard using vanilla web technologies. Designed for researchers, the interface abstracts away the underlying API complexity, allowing users to:
\begin{itemize}
  \item \textbf{Manage Elections:} Initialize elections with tunable parameters (threshold bounds, authority count, grace period).
  \item \textbf{Simulate Cohorts:} Perform batch voter registration and automated ballot casting to quickly populate the system state.
  \item \textbf{Inspect Data:} Browse the public bulletin board (Poseidon Merkle tree), encrypted ballot log, and raw backend state files in real time.
  \item \textbf{Test Coercion Resistance:} Issue fake casting keys and generate simulated deniability proofs (\textsf{SimKeyProve}) interactively.
  \item \textbf{Execute Audits:} Trigger complex cryptographic events such as $\pi_{\text{total}}$ verification, multi-authority threshold tallies, and nullification-based censorship audits with a single click.
\end{itemize}

The dashboard maintains a real-time protocol log, capturing every cryptographic operation, API request, and state transition, thereby significantly simplifying the evaluation of the protocol's E2E verifiability and coercion-resistance guarantees.

% Uncomment and add a screenshot of the dashboard named 'dashboard.png' into the Final_Report folder.
% \begin{figure}[htbp]
% \centerline{\includegraphics[width=\columnwidth]{dashboard.png}}
% \caption{The zkVoting Research Simulation Dashboard displaying protocol state, interactive voter sessions, and the real-time cryptographic log.}
% \label{fig:dashboard}
% \end{figure}
...
\begin{enumerate}
  \item \textbf{Trusted setup (A5)}: Groth16 requires a per-circuit trusted setup.
    Migrating to Plonk or Halo2 would eliminate this assumption.
    \emph{Not implemented}: requires a fundamentally different ZK backend;
    circuit conversion is thesis-level effort.

  \item \textbf{Blockchain bulletin board (A10)}: the prototype uses JSON files.
    A tamper-evident ledger (e.g.\ an Ethereum smart contract) is required
    for production deployment and full E2E verifiability.
    \emph{Not implemented}: engineering effort orthogonal to cryptographic
    contributions; well-understood integration path.

  \item \textbf{Sybil prevention (A1)}: the protocol lacks on-chain identity
    verification.  \emph{Proposed solution}: bind each Merkle leaf to a
    unique credential hash $\mathit{credHash} = H(\mathit{national\_id},
    e)$, rejecting duplicate credential hashes.  Privacy is preserved via
    the hash.  \emph{Not implemented}: requires integration with an external
    identity provider and raises policy questions beyond the cryptographic
    scope of this work.

  \item \textbf{Private channel assumption (A2)}: coercion resistance
    assumes the voter has an ``untappable'' private channel for re-voting.
    This is a physical-world constraint that no cryptographic protocol can
    fully solve.  Practical mitigations include anonymous communication
    networks (Tor), in-person re-vote kiosks, or time-delayed re-vote
    submission.  \emph{Not implemented}: inherently non-cryptographic.

  \item \textbf{Quantum resistance (A7)}: all security reductions depend on
    DDH/DLP hardness over Baby~Jubjub.  A sufficiently powerful quantum
    computer breaks these assumptions.  \emph{Proposed solution}: migrate
    to lattice-based commitments and hash-based ZK (e.g.\ STARKs).
    \emph{Not implemented}: post-quantum ZK is an active research frontier
    with no production-ready toolchain compatible with Circom.

  \item \textbf{Poseidon random oracle model (A9)}: the security proofs
    model Poseidon as a random oracle.  If algebraic attacks exploit
    Poseidon's structure, commitment binding and collision resistance
    may degrade.  \emph{Not implemented}: would require either formal
    cryptanalysis or a hash-function swap (e.g.\ to Rescue), both outside
    the scope of a systems-level contribution.

  \item \textbf{Synchronous phases (A12)}: the paper assumes strict phase
    boundaries.  Late arrivals to the registration or voting phase are
    excluded.  Improvement~VI partially addresses this for the voting phase;
    registration-phase flexibility remains future work.

  \item \textbf{Tally scaling (A13)}: the $O(n)$ nullification tally uses
    Baby~Jubjub EC scalar multiplication per ballot.  For national-scale
    elections ($> 10^7$ voters), this may require parallelisation or
    batch verification optimisations.

  \item \textbf{Encryption proof in circuit}: we prove only the Poseidon
    commitment in-circuit; the hybrid encryption $\mathit{CT} =
    \Pi_{\mathrm{HBE}}.\mathsf{Enc}(\mathit{pk}, m; k)$
...
cryptographic components and identified six novel improvements.  The
...
Ephemeral epoch secrets provide forward secrecy against retroactive
deanonymisation.  Commit-reveal registration makes authority censorship
publicly detectable.  A hidden grace period closes timing-based coercion
attack vectors.

Our systematic assumption analysis identified 13 security assumptions in the
original protocol, of which our six improvements address five.  For the
remaining eight, we provide proposed solutions and justify their exclusion
...
zkVoting paper on six additional dimensions.

% ─────────────────────────────────────────────────────────────────────────────
\section*{Responsible Disclosure of AI Assistance}
\label{sec:disclosure}
% ─────────────────────────────────────────────────────────────────────────────

In the interest of academic transparency, the authors disclose the use of
Large Language Models (LLMs) and agentic AI tools during the development
of this project. AI assistance was utilized primarily to accelerate the
rapid prototyping of the web dashboard (HTML/CSS layout generation and DOM
scripting), scaffolding the Express.js backend boilerplate, and to keep documentation.

All core cryptographic logic, including the circomlibjs nullifiable
commitment implementations, the zero-knowledge circuit modifications
in \texttt{enhanced\_vote.circom}, and the theoretical assumption analysis
were meticulously designed, directed, and mathematically checked by the primary
authors. The inclusion of AI-assisted scripting strictly served to expedite
empirical validation and UI construction without compromising the academic
integrity or theoretical rigor of the core research.
